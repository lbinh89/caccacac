...
export default class Controller {
    ...
    async registerMailToListController(req, res) {
        ...
        this.requireParamsValidation(req, res, dataInput);

        // check format name, address
        this.formatParamsValidation(req, res, dataInput);

        // check mail exists
        this.checkInputMailExists(req, res, dataInput);
        ...
        try {
            ...
            if (listMailExists.length > 0) {
                ...
                return res.status(config.httpStatus.IS_EXIST.status)
                    .json({
                        inquiryId: uuidv4(),
                        error: {
            ...
            if (dataRegister.length > 0) {
                return res.status(config.httpStatus.CREATED.status).json(dataRegister);
            }
        } catch (e) {
            return res.status(config.httpStatus.INTERNAL_SERVER.status)
                .json({
     ...
     * @returns {boolean}
     */
    checkInputMailExists(req, res, data) {
        let listMail = [];
        let listMailExists = [];
        let outputMsgExists = errorsMessage.AlreadyExist;
